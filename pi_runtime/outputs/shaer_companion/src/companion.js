...
(function () {
  ...
  function showView(view) {
    ...
    if (view === "feedback") updateFeedbackPreview();
  ...
  async function discover() {
    ...
    setAppState("connecting", "Finding SHAeR", "Searching nearby over Bluetooth and the local network.");
    try {
      ...
      setAppState("offline", "SHAeR unavailable", "Keep SHAeR nearby and make sure both devices are connected to the same Wi-Fi network.");
  ...
  async function showDeviceDiscovery() {
    ...
    root.textContent = "Searching nearby over Bluetooth...";
    if (!dialog.open) dialog.showModal();
    const devices = await discovery.scan(state.baseUrl);
    if (!devices.length) {
      root.textContent = "No nearby SHAeR was found. Check Bluetooth and Wi-Fi, or use the network-address fallback below.";
      return;
    }
    root.className = "discovered-devices";
    root.innerHTML = devices.map((device) => `<button class="discovered-device secondary" type="button" data-device-url="${escapeHtml(device.baseUrl)}"><span>${escapeHtml(device.device_name || "SHAeR")}</span><small>${device.transport === "bluetooth" ? "Found nearby via Bluetooth" : "Found on local network"}</small></button>`).join("");
  }
  ...
  async function loadBranding() {
    try {
      const branding = await api("/api/v1/branding");
      const icon = $("#brandIcon");
      if (icon && branding.icon_url) {
        icon.src = client.url(branding.icon_url);
        icon.hidden = false;
      }
    } catch {
      // Branding is cosmetic; connection state should stay governed by real device health.
    }
  }
  ...
  async function loadThemes() {
    try {
      ...
      const previews = {
        archive_dark: ["#06100d", "#00d89b"],
        bombay_ticket: ["#ddd7c8", "#b84c5c"],
        japanese_punk: ["#0d0d10", "#ed0060"],
        windows_xp: ["#5b93d8", "#f1f1e8"],
        ghibli_garden: ["#5c7d78", "#f1d7bd"],
        indian_print: ["#600000", "#ead9b8"],
        indian_raga: ["#600000", "#ead9b8"]
      };
      const node = $("#themeGrid");
      node.className = "theme-grid";
      node.innerHTML = (data.installed || []).map((theme) => {
        const [preview, accent] = previews[theme.id] || ["#202426", "#d8e1df"];
        return `<article class="theme-card ${theme.id === data.active ? "active" : ""}"><div class="theme-preview" style="--preview:${preview};--accent:${accent}"></div><footer><div><strong>${escapeHtml(theme.name)}</strong><small>${theme.id === data.active ? "Active" : "Installed"}</small></div><button type="button" data-theme="${escapeHtml(theme.id)}">${theme.id === data.active ? "Active" : "Use"}</button></footer></article>`;
      }).join("");
    } catch (error) { toast(error.message, true); }
  }
  ...
  async function loadArchive() {
    try {
      const data = await api("/api/v1/archive");
      ...
      if (!state.archiveEntries.length) {
        ...
        node.textContent = "No Marginalia yet. Leave a handwritten mark from SHAeR's Now Playing screen.";
        return;
      }
      node.className = "recording-list";
      node.innerHTML = state.archiveEntries.map((entry) => {
        const pages = (entry.marginalia || []).map((page) => `<img loading="lazy" src="${escapeHtml(`${state.baseUrl}${page.image_path}`)}" alt="Marginalia page written at ${escapeHtml(formatMilliseconds(page.playback_position_ms))}">`).join("");
        return `<article class="recording-row archive-entry"><div><strong>${escapeHtml(entry.title)}</strong><small>${escapeHtml([entry.artist, entry.album].filter(Boolean).join(" · "))} · ${entry.marginalia_count} page${entry.marginalia_count === 1 ? "" : "s"}</small></div><div class="archive-pages">${pages || "<span class=\"muted\">No pages</span>"}</div></article>`;
      }).join("");
    } catch (error) { toast(error.message, true); }
  }
  ...
  function feedbackContext(extra = {}) {
    ...
    const storage = dashboard.storage || {};
    return {
      shaer_os_version: dashboard.firmware_version || dashboard.shaer_os_version || $("#miniFirmware").textContent || "unknown",
      theme: dashboard.current_theme || state.settings?.display?.theme || "unknown",
      device_serial: dashboard.device_id || state.deviceId || "unknown",
      battery_percent: dashboard.battery_percent ?? null,
      storage_remaining: storage.free ?? null,
      spotify_connected: Boolean(dashboard.spotify_authenticated),
      bluetooth_connected: Boolean(extra.bluetooth_connected),
      wifi_ssid: extra.wifi_ssid || "unknown",
      companion_version: "phone-0.19.0",
      connection: $("#connectionLabel").textContent,
      user_agent: navigator.userAgent
    };
  }

  function updateFeedbackPreview(context = feedbackContext()) {
  ...
  async function sendFeedback() {
    ...
    if (!message) return toast("Tell SHAeR what happened first.", true);
    $("#feedbackState").textContent = "Sending...";
    const [wifi, bluetooth] = await Promise.allSettled([api("/api/v1/network/wifi"), api("/api/v1/bluetooth")]);
    const context = feedbackContext({
      wifi_ssid: wifi.status === "fulfilled" ? wifi.value.ssid : "unknown",
      bluetooth_connected: bluetooth.status === "fulfilled" ? Boolean((bluetooth.value.paired_devices || []).length) : false
    });
    updateFeedbackPreview(context);
    try {
      await api("/api/v1/feedback", { method: "POST", body: { message, severity: $("#feedbackSeverity").value, context } });
      $("#feedbackMessage").value = "";
      $("#feedbackState").textContent = "Sent";
      toast("Feedback sent to SHAeR.");
    } catch (error) {
      $("#feedbackState").textContent = "Send failed";
      toast(error.message, true);
    }
  }
  ...
  $("#feedbackMessage").addEventListener("input", () => updateFeedbackPreview());
